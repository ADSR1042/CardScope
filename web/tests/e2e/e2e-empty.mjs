...
try {
  ...
  await page.screenshot({ path: runtimePath('settings-empty.png'), fullPage: true });
  console.log('Logout');
  await page.getByRole('button', { name: '退出登录' }).click();
  await page.locator('.login-art h1').filter({ hasText: '实验室 GPU' }).waitFor();
  assert((await page.locator('.login-art p').textContent()) === '', 'Plain text mismatch');
  assert((await page.locator('.login-caption').textContent()) === '', 'Blank caption not hidden');
  await page.screenshot({ path: runtimePath('login-empty.png'), fullPage: true });
